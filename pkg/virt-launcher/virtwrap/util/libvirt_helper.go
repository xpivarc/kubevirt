...
func (l LibvirtWraper) SetupLibvirt() error {
	...
	libvirdDConf, err := os.OpenFile("/etc/libvirt/libvirtd.conf", os.O_RDONLY, 0644)
	if err != nil {
		return err
	}
	...
	_, err = io.Copy(newLibvirtConf, libvirdDConf)
	if err != nil {
		return err
	}
	...
	if envVarValue, ok := os.LookupEnv("LIBVIRT_DEBUG_LOGS"); ok && (envVarValue == "1") {
		...
		_, err = newLibvirtConf.WriteString("log_filters=\"3:remote 4:event 3:util.json 3:util.object 3:util.dbus 3:util.netlink 3:node_device 3:rpc 3:access 1:*\"\n")
		if err != nil {
			return err
		}
	}
